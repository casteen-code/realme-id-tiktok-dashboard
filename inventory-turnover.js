(() => {
  ...
  const COLOR_PHRASES = ['titanium silver','titanium black','lavender purple','moss green','ivory gold','white swan','forest owl','swan black','kingfisher blue','parrot purple','peacock green','phantom navy','rally white','glacier blue','storm black','dusk gray','dawn purple','master gray','master grey','master gold','pebble grey','pebble gray','orchid purple','air white','air black','pulse purple','glory beige','deepblue tides','deepblue tide','brown','black','white','blue','purple','green','gray','grey','gold','silver','red','orange'];
  ...
  function mergeRules(saved) {
    ...
    return { modelAliases: { ...base.modelAliases, ...(saved?.modelAliases || {}) }, colorAliases: { ...base.colorAliases, ...(saved?.colorAliases || {}) }, manual: { ...(saved?.manual || {}) } };
  }
  ...
  function model(value) {
    let s = clean(value).replace(/\brealme\b/ig, ' ').replace(/\b(?:4|6|8|12|16)\s*(?:gb|g)?\s*[\/x×+\-]\s*\d{2,4}\s*(?:gb|g)?\b/ig, ' ');
    const sixteen = s.match(/\b16\s*(?:pro\s*\+?)?(?![a-z0-9])/i); if (sixteen) { const v = key(sixteen[0]); return v.includes('pro') ? (v.includes('plus') ? '16 Pro+' : '16 Pro') : '16'; }
    const m = s.match(/\b(note\s*\d{1,3}(?:\s*(?:pro|plus|x|t|s))?|c\s*\d{1,3}(?:\s*(?:i|x|s|a|pro|plus))?|p\s*\d{1,3}(?:\s*(?:pro|plus|x|t|s))?|narzo\s*\d{1,3}(?:\s*(?:pro|plus|x|t|s))?|buds\s*(?:air\s*)?\d{0,3}(?:\s*(?:pro|neo|t|s|plus))?|watch\s*\d*[a-z]*)\b/i);
    let out = (m ? m[1] : s.split(/[|,，;]/)[0]).replace(/\b(?:128|256|512)\s*(?:gb|g)\b/ig, ' ').replace(/\s+/g, ' ').trim();
    out = out.replace(/\bpro\b/ig, 'Pro').replace(/\bplus\b/ig, 'Plus').replace(/\bnote\s*/ig, 'Note ').replace(/\bc\s*/ig, 'C').replace(/\bp\s*/ig, 'P');
    return aliases('model')[key(out)] || out || '未识别型号';
  }
  ...
  function parseSKU(raw, parts = {}) {
    const label = [parts.model || raw, parts.memory, parts.color].filter(Boolean).join(' | ');
    const m = model(label), mem = memory(parts.memory || label), col = color(parts.color || label);
    return { raw: clean(label), rawKey: key(label), model: m, modelKey: key(m), memory: mem, color: col, fullKey: [key(m), key(mem), key(col)].join('|'), baseKey: [key(m), key(mem)].join('|') };
  }
  ...
  function stocksFromInputs() { const out = []; for (const slot of ['mks','pnk','bali']) { const s = profile(slot), m = s.mapping; for (const r of s.records) { const raw = value(r, m.sku); if (!raw || isSummary(raw)) continue; out.push({ sku: parseSKU(raw), warehouse: slot, qty: Math.max(0, num(value(r, m.qty))), source: SLOT[slot].label }); } } return out; }
  ...
  function calculate(save = true) {
    const stocks = stocksFromInputs(), sales = salesFromInputs(), issues = matchSales(stocks, sales); const modelRows = groupRows(stocks, sales, 'model', 'all'), skuRows = groupRows(stocks, sales, 'sku', 'all');
    app.historySnapshot = null; app.result = { stocks, sales, issues, modelRows, skuRows, createdAt: Date.now(), audit: auditSnapshot() }; $('report').classList.remove('hidden'); renderReport(); if (save) saveSnapshot(); setStatus(`已完成：${stocks.length} 条库存、${sales.length} 条销量`, issues.length ? 'warn' : 'normal'); show(issues.length ? `已计算。仍有 ${issues.length} 条销售 SKU 需要确认；它们不会被悄悄合并。` : '已计算，所有销售 SKU 都已匹配。', issues.length ? 'warn' : 'ok');
  }
  ...
  function renderRules() {
    ...
    document.querySelectorAll('[data-rule-type]').forEach(btn => btn.onclick = async () => { delete app.rules[btn.dataset.ruleType][btn.dataset.ruleKey]; await saveRules(); });
  }
